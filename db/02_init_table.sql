...
(
   ...
   PRIORITY             INT COMMENT '菜单顺序',
   URL                  CHAR(200) COMMENT '是否有效（1=有效，0=无效）',
   PARENT_ID            INT COMMENT '菜单父ID',
   IS_MENU              CHAR(1) COMMENT '是否是菜单（1=菜单，0=按钮）',
...
(
   ...
   SYSTEM_CODE          CHAR(1) COMMENT '角色所属系统代码(0=前台系统，1=后台系统)',
   CREATE_BY            INT COMMENT '创建者ID',
   CREATE_TIME          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   MODIFY_BY            INT COMMENT '修改者ID',
   MODIFY_TIME          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间',
